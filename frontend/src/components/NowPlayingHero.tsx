...
function HeroCover({ song }: { song: Song }) {
  ...
  return (
    <Link
      href={`/songs/${song.id}`}
      className="relative grid aspect-square w-full max-w-[220px] shrink-0 place-items-center overflow-hidden rounded-2xl border border-zinc-700/80 bg-gradient-to-br from-green-500 to-zinc-950 shadow-2xl shadow-black/40 sm:max-w-[260px] lg:w-60"
        ...
        <div className="min-w-0 flex-1 space-y-5">
          <div className="min-w-0">
            ...
            <Link href={`/songs/${song.id}`} className="group mt-2 block">
              <h1 className="truncate text-3xl font-extrabold text-white transition group-hover:text-green-400 sm:text-5xl">
                {song.title}
              </h1>
            </Link>
            <div className="mt-3 flex flex-wrap items-center gap-2 text-sm text-zinc-400 sm:text-base">
              <Link
                ...
                className="font-semibold text-zinc-200 transition hover:text-green-400"
              >
                {song.artist.name}
              </Link>
